...
@section('content')
    <div class="col-md-8">
    <form class="form-signin" action="{{ route('expense.update', $expense->id) }}" method="post">
        @csrf
        {{ method_field('put') }}
        <h1 class="h3 mb-3 font-weight-normal">الرجاء إدخال البيانات</h1>

        <input type="text" class="form-control" name="name" value="{{ $expense->name }}" placeholder="الاسم" required>
        <input type="text" class="form-control" name="price" value="{{ $expense->price }}" placeholder="المبلغ" required>
